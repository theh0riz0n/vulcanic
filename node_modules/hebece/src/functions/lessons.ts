...
export default async (keyPair:KeyPair, restUrl: string, pupil:Pupil, dateFrom: Date, dateTo: Date) => {
	...
	const url = `${strings.BASE_URL}/${tenant}/${pupil.Envelope[0].Unit.Symbol}/api/mobile/schedule/byPupil?unitId=${pupil.Envelope[0].Unit.Id}&pupilId=${pupil.Envelope[0].Pupil.Id}&periodId=${currentPeriod.Id}&dateFrom=${moment(dateFrom).format('YYYY-MM-DD')}&dateTo=${moment(dateTo).format('YYYY-MM-DD')}&pageSize=100`;
	const date = new Date();
	...
	// @ts-ignore
	const data:Lesson = await aab.json();
	handleErrors(data);
